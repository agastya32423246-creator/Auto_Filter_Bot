...
API_ID = int(environ.get('API_ID', '15554219')) # API ID from my.telegram.org
API_HASH = environ.get('API_HASH', 'f7e1e318df2bfa9ba39b14e34c43fb4c')  # API Hash from my.telegram.org
BOT_TOKEN = environ.get('BOT_TOKEN', "8616081741:AAGJyCkuY64LsATfcwiy39DMvA5vcvfpp0c")    # Bot token from @BotFather
...
# ============================
ADMINS = [int(admin) if id_pattern.search(admin) else admin for admin in environ.get('ADMINS', '1463212098').split()] # Replace with the actual admin ID(s) to add
CHANNELS = [int(ch) if id_pattern.search(ch) else ch for ch in environ.get('CHANNELS', '-1004298873390').split()]  # Channel id for auto indexing (make sure bot is admin)

LOG_CHANNEL = int(environ.get('LOG_CHANNEL', '-1004298873390'))  # Log channel id (make sure bot is admin)
BIN_CHANNEL = int(environ.get('BIN_CHANNEL', '-1004298873390'))  # Bin channel id (make sure bot is admin)
PREMIUM_LOGS = int(environ.get('PREMIUM_LOGS', '-1004298873390'))  # Premium logs channel id
DELETE_CHANNELS = [int(dch) if id_pattern.search(dch) else dch for dch in environ.get('DELETE_CHANNELS', '-1004298873390').split()] #(make sure bot is admin)
...
SUPPORT_CHAT = environ.get('SUPPORT_CHAT', 'https://t.me/NRXfilmsgroup')  # Support group link (make sure bot is admin)
...
auth_channels     = environ.get("AUTH_CHANNELS", "-1003909147235")# Channels for force sub (make sure bot is admin)
...
# ============================
DATABASE_URI = environ.get('DATABASE_URI', "mongodb+srv://tapanagastya5:iIAPWgSW8h5ysSVz@helper2.un901le.mongodb.net/?appName=Helper2")  # MongoDB URI for the database
...
MOVIE_UPDATE_CHANNEL = int(environ.get('MOVIE_UPDATE_CHANNEL', '-1003909147235'))  # Notification of sent to your channel
